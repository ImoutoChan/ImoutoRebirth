...
| DanbooruUserId                            |          | Крайне желательно заполнить, числовой ID вашего пользователя на danbooru: строчка ID в начале страницы профиля (или число в адресе профиля https://danbooru.donmai.us/users/<id>). Danbooru требует его в user agent запросов к api, без него часть запросов отвечает 403 |
